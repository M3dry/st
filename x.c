...
cresize(int width, int height)
{
	...
	col = (win.w - 2 * borderperc) / win.cw;
	row = (win.h - 2 * borderperc) / win.ch;
	col = MAX(1, col);
	row = MAX(1, row);
...
xhints(void)
{
	...
	sizeh->height_inc = 1;
	sizeh->width_inc = 1;
	sizeh->base_height = 2 * borderperc;
	sizeh->base_width = 2 * borderperc;
	sizeh->min_height = win.ch + 2 * borderperc;
	sizeh->min_width = win.cw + 2 * borderperc;
	if (xw.isfixed) {
...
xloadfonts(char *fontstr, double fontsize)
{
	...
	borderperc = ceilf(((float)borderperc / 100) * win.cw);
...
xinit(int cols, int rows)
{
	...
	win.w = 2 * win.hborderpx + 2 * borderperc + cols * win.cw;
	win.h = 2 * win.vborderpx + 2 * borderperc + rows * win.ch;
	if (xw.gm & XNegative)
...
xximspot(int x, int y)
{
	...
	xw.ime.spot.x = borderperc + x * win.cw;
	xw.ime.spot.y = borderperc + (y + 1) * win.ch;
